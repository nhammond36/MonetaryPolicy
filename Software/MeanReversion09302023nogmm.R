...
spread$TargetUe %>% replace(is.na(.),0)
spread$IORR %>% replace(is.na(.),0)
spread$RRPONTSYAWARD %>% replace(is.na(.),0)


#library(dplyr)
names(spread)
# [1] "Time"          "EFFR"          "VolumeEFFR"    "TargetUe"      "TargetDe"      "PercentileE1"  "PercentileE25" "PercentileE75" "PercentileE99" "OBFR"         
# [11] "VolumeOBFR"    "PercentileO1"  "PercentileO25" "PercentileO75" "PercentileO99" "TGCR"          "VolumeTGCR"    "PercentileT1"  "PercentileT25" "PercentileT75"
# [21] "C"             "BGCR"          "VolumeBGCR"    "PercentileB1"  "PercentileB25" "PercentileB75" "PercentileB99" "SOFR"          "VolumeSOFR"    "sTargetD"     
# [31] "sTargetU"      "PercentileS1"  "PercentileS25" "PercentileS75" "PercentileS99" "IORR"          "RRPONTSYAWARD"

# This code uses the mutate function to replace NA values in the PercentileE1 column with zeros. 
spread <- spread %>% 
  mutate(PercentileE1 = ifelse(is.na(PercentileE1), 0, PercentileE1))
# Access the modified PercentileE1 column within the spread data frame
# spread$PercentileE1
spread <- spread %>% 
  mutate(PercentileE25 = ifelse(is.na(PercentileE25), 0, PercentileE25))
spread <- spread %>% 
  mutate(PercentileE75 = ifelse(is.na(PercentileE75), 0, PercentileE75))
spread <- spread %>% 
  mutate(PercentileE99 = ifelse(is.na(PercentileE99), 0, PercentileE99))

spread <- spread %>% 
  mutate(PercentileS1 = ifelse(is.na(PercentileS1), 0, PercentileS1))
spread <- spread %>% 
  mutate(PercentileS25 = ifelse(is.na(PercentileS25), 0, PercentileS25))
spread <- spread %>% 
  mutate(PercentileS75 = ifelse(is.na(PercentileS75), 0, PercentileS75))
spread <- spread %>% 
  mutate(PercentileS99 = ifelse(is.na(PercentileS99), 0, PercentileS99))
...
# The standard deviation can be estimated from percentiles assuming a specific distribution. A common assumption for interest rates is a log-normal distribution. If you have data on percentiles (e.g., 25th and 75th percentiles), you can estimate the standard deviation (
# σ) ≈75th percentile−25th percentile/2× inverse standard normal cumulative distribution function of 0.75
#  
# Here, the inverse standard normal cumulative distribution function of 0.75 corresponds to the z-score for the 75th percentile.
# 
# Estimate Skewness:
# Skewness can be estimated assuming the log-normal distribution. For a log-normal distribution, skewness (
# S) is related to the standard deviation (σ) by the following formula:
# S≈ 3×(mean−median)/σ

# Estimate Kurtosis:
# Kurtosis can also be estimated assuming the log-normal distribution. For a log-normal distribution, excess kurtosis (
# K) is related to the standard deviation (
# σ) by the following formula:
# K+  (mean−median)/σ^2 

# The excess kurtosis subtracts 3 (the excess kurtosis of a normal distribution) to provide a measure of kurtosis relative to a normal distribution.
# Please note that these estimations are based on assumptions about the distribution of interest rates and may not perfectly represent the true distribution. The accuracy of these estimations relies on the validity of the distribution assumption. It's always advisable to consult with a financial statistician or expert in the field to discuss the appropriateness of these assumptions and estimations for your specific context.


# Data frames 
# rrbp overnight interest rates
#selected_cols <- spread[, c("EFFR", "OBFR", "TGCR", "BGCR", "SOFR" )]
# Specify the column names you want to select
selected_col_names <- c("EFFR", "OBFR", "TGCR", "BGCR", "SOFR")
# Select the columns based on the specified column names
selected_cols <- spread[begs:ends, selected_col_names]
# Create a new data frame with the selected columns
rrbp <- data.frame(selected_cols)*100
# Print the first few rows of the new data frame
head(rrbp)
rrbp %>% replace(is.na(.),0)

# vold daily volumes 
selected_vold <- spread[, c("VolumeEFFR", "VolumeOBFR", "VolumeTGCR", "VolumeBGCR", "VolumeSOFR" )]
selected_vold_names <- c("VolumeEFFR", "VolumeOBFR", "VolumeTGCR", "VolumeBGCR", "VolumeSOFR")
selected_vold <- spread[begs:ends, selected_vold_names]
vold<- data.frame(selected_vold)
head(vold)
...
# quantile daily volumes 
#selected_quantiles <- spread[, c("PercentileE1","PercentileE25","PercentileE75","PercentileE99","PercentileO1", "PercentileO25","PercentileO75","PercentileO99","PercentileT1","PercentileT25", "PercentileT75", "PercentileT99",  "PercentileB1",  "PercentileB25",  "PercentileB75",  "PercentileB99", "Percentiles1","Percentiles25", "Percentiles75", "Percentiles99")]
selected_quantilesE <- spread[, c("EFFR","TargetUe","TargetDe","PercentileE1","PercentileE25","PercentileE75","PercentileE99")]
selected_quantilesO <- spread[, c("OBFR","PercentileO1","PercentileO25","PercentileO75","PercentileO99")]
selected_quantilesT <- spread[, c("TGCR","PercentileT1","PercentileT25","PercentileT75","PercentileT99")]
selected_quantilesB <- spread[, c("BGCR","PercentileB1","PercentileB25","PercentileB75","PercentileB99")]
selected_quantiless <- spread[, c("SOFR","PercentileS1","PercentileS25","PercentileS75","PercentileS99")]

selected_quantiles_namesE <- c("EFFR","TargetUe","TargetDe","PercentileE1","PercentileE25","PercentileE75","PercentileE99")
selected_quantiles_namesO <- c("OBFR","PercentilO1","PercentileO25","PercentileO75","PercentileO99")
selected_quantiles_namesT <- c("TGCR","PercentileT1","PercentileT25","PercentileT75","PercentileT99")
selected_quantiles_namesB <- c("BGCR","PercentileB1","PercentileB25","PercentileB75","PercentileB99")
selected_quantiles_namesS <- c("SOFR","PercentileS1","PercentileS25","PercentileS75","PercentileS99")

selected_quantilesE <- spread[begs:ends, selected_quantiles_namesE]*100
head(quantilesE)
#selected_quantilesO <- spread[begs:ends, selected_quantiles_namesO]*100
head(quantilesO)
selected_quantilesT <- spread[begs:ends, selected_quantiles_namesT]*100
head(quantilesT)
selected_quantilesB <- spread[begs:ends, selected_quantiles_namesB]*100
head(quantilesB)
selected_quantilesS <- spread[begs:ends, selected_quantiles_namesS]*100
head(quantilesS)




# qq plots of overnight rates -------------------------
# Chatgpt request
# dataframe quantilesE<- data.frame(selected_quantilesE)*100 contains perecentiles of EFFR "PercentileE1","PercentileE25","PercentileE75","PercentileE99"
# make a qq plot using ggplot of percentiles "PercentileE1","PercentileE25","PercentileE75","PercentileE99" versus a normal distribution
# Load the ggplot2 library
library(ggplot2)

# Create a data frame with your percentiles
# percentiles <- data.frame(
#   PercentileE1 = c(1st_percentile_value1, 1st_percentile_value2, ...),
#   PercentileE25 = c(25th_percentile_value1, 25th_percentile_value2, ...),
#   PercentileE75 = c(75th_percentile_value1, 75th_percentile_value2, ...),
#   PercentileE99 = c(99th_percentile_value1, 99th_percentile_value2, ...)
# )
...
ggplot(data = selected_quantilesE, aes(sample = PercentileE1)) +
  ...
  #qqline(PercentileE1, col = "steelblue", lwd = 2) +
  labs(x = "Theoretical Quantiles", y = "Sample Quantiles") +
  ggtitle("QQ Plot of EFFR PercentileE1 vs Normal Distribution")

ggplot(data = selected_quantilesE, aes(sample = PercentileE25)) +
  geom_qq(distribution = qnorm, color = "blue") +
  geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
  labs(x = "Theoretical Quantiles", y = "Sample Quantiles") +
  ggtitle("QQ Plot of EFFR PercentileE25 vs Normal Distribution")

ggplot(data = selected_quantilesE, aes(sample = PercentileE75)) +
  geom_qq(distribution = qnorm, color = "blue") +
  geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
  labs(x = "Theoretical Quantiles", y = "Sample Quantiles") +
  ggtitle("QQ Plot of EFFR PercentileE75 vs Normal Distribution")


ggplot(data = selected_quantilesE, aes(sample = PercentileE99)) +
  geom_qq(distribution = qnorm, color = "blue") +
  geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
  labs(x = "Theoretical Quantiles", y = "Sample Quantiles") +
  ggtitle("QQ Plot of EFFR PercentileE99 vs Normal Distribution")
...
# box plots
boxplot(spread$EFFR, 
        main = "Boxplot of EFFR",
        xlab = "EFFR",
        col = "lightblue",
        border = "blue",
        horizontal = TRUE)  # This makes it a horizontal boxplot

boxplot<-ggplot(rrbp,aes(sdate,rrbp))

# Create a sample data frame with five series
data <- data.frame(
  Series1 = rnorm(50, mean = 0, sd = 1),
  Series2 = rnorm(50, mean = 1, sd = 1),
  Series3 = rnorm(50, mean = 2, sd = 1),
  Series4 = rnorm(50, mean = 3, sd = 1),
  Series5 = rnorm(50, mean = 4, sd = 1)
)
...
# metricE<-select(spread[begs:ends,], EFFR,TargetUe,TargetDe,PercentileE1,PercentileE25,PercentileE75,PercentileE99)
# metricS<-select(spread, SOFR,TargetUe,TargetDe,PercentileS1,PercentileS25,PercentileS75,PercentileS99)
# metricE<-metricE*100
# metricS<-metricS*100
...
#spreads
k<-1:5
iorsofr<-(spread$IORR*100-rrbp[,k[5]])
ioreffr<-(spread$IORR*100-rrbp[,k[1]])
rrppsofr<-spread$RRPONTSYAWARD-rrbp[,k[5]];

#quantileE<-select(spread,PercentileE1,PercentileE25,PercentileE75,PercentileE99) #,TargetDE,TargetUE)
#quantileE<-select(spread,spread$PercentileE1,spread$PercentileE25,spread$PercentileE75,spread$PercentileE99) #,TargetDE,TargetUE)
# quantileE1<-spread$PercentileE1*100
# quantileE25<-spread$PercentileE25*100
# quantileE75<-spread$PercentileE75*100
# quantileE99<-spread$PercentileE99*100
# # problem quantileE<-c(quantileE1,quantileE25,quantileE75.quantileE99)
# # quantileeffr<-spread(1:1711,5:8);
# quantileobfr<-spread(1:1711,13:16); 
...
effrrates <- ggplot(metricE, aes(x = sdate)) +
  ...
  scale_y_continuous(breaks = y_breaks, limits = y_limits)
print(effrrates)
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/effrratesline.pdf")
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/effrratesline.png")

# note outlier 9/17/2019 5.25	1177			2.25	5	5.85	9
# rate, volume, percentiles
# Assuming metricS is a data frame
# Identify rows where the first column is greater than 5
rows_to_update <- metricS[, 1] > 5
# Set columns 4 to 7 to 0 for the identified rows
metricS[rows_to_update, 4:7] <- 0

sofrrates <- ggplot(metricS, aes(x = sdate)) +
  geom_line(aes(y = metricS[,1], color = "SOFR"), linewidth =1) + 
  geom_line(aes(y = metricS[,4], color = "PercentileS1",alpha = 0.5),  linewidth = 1) + 
  geom_line(aes(y = metricS[,5], color = "PercentileS25",alpha = 0.5), linewidth = 1) + 
  geom_line(aes(y = metricS[,6], color = "PercentileS75",alpha = 0.5), linewidth = 1) + 
  geom_line(aes(y = metricS[,7], color = "PercentileS99",alpha = 0.5), linewidth = 1) + 
  labs(x = "Date", y = "basis points (bp)", color = "Lines") + 
  scale_color_manual(values = c("SOFR" = "black", "PercentileS1" = "yellow", "PercentileS25"= "green","PercentileS75" = "blue","PercentileS99" = "red")) + 
  theme_minimal() +
  scale_y_continuous(breaks = y_breaks, limits = y_limits)
print(sofrrates)
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/sofrratesline.pdf")
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/sofrratesline.png")
...
grid_sample <- grid.arrange(effrrates, sofrrates,securedrates,ncol = 3)
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/sample.png", grid_epsiodes, width = 12, height = 8, dpi = 300)
...
dailyrates<-ggplot(rrbp, aes(x = sdate)) +
  ...
  theme_minimal()
print(dailyrates)
unsecuredrates <- ggplot(rrbp, aes(x = sdate)) +
  geom_line(aes(y = rrbp[,1], color = "EFFR"), linewidth =1) + 
  geom_line(aes(y = rrbp[,2], color = "OBFR"), linewidth = 1) + 
  #geom_line(aes(y = rrbp[,3], color = "TGCR"), linewidth = 1) + 
  #geom_line(aes(y = rrbp[,4], color = "BGCR"), linewidth = 1) + 
  #geom_line(aes(y = rrbp[,5], color = "SOFR"), linewidth = 1) + 
  labs(x = "Date", y = "basis points (bp)", color = "Lines") + 
  scale_color_manual(values = c("EFFR" = "black", "OBFR" = "blue")) + 
  theme_minimal()
print(unsecuredrates)
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/unsecuredratesline.pdf")
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/unsecuredratesline.png")


# Sample stats           
stats_metricE<-colMeans(metricE)
stats_metricS<-colMeans(metricS)

p2575E<-stats_metricE[6]-stats_metricE[5]
p0199E<-stats_metricE[7]-stats_metricE[4]
p2575S<-stats_metricS[6]-stats_metricS[5]
p0199S<-stats_metricS[7]-stats_metricS[4]


inverse_value <- qlnorm(0.75) #  1.963031
print(inverse_value)
sde<-p2575E/(2*inverse_value) #  0.4738036 
sds<-p2575S/(2*inverse_value) #   -9.464567 
print(sde)
print(sds)

skewe<-3*(p2575E-2*stats_metricE[1])/(2*inverse_value) #  -164.1402 
kurte<-p2575E/(inverse_value*sde) # 2
print(skewe)
print(kurte)


skews<-3*(p2575S-2*stats_metricS[1])/(2*inverse_value) #    -165.2238 
kurts<-p2575S/(inverse_value*sde) #   0.04471033 
print(skews)
print(kurts)

stats_metricE$sd <- sde
stats_metricS$sd <- sds

stats_metricE$skew <- skewe
stats_metricS$skew <- skews

stats_metricE$kurtosis <- kurte
stats_metricS$kurtosis <- kurts
...
stats_sofr <- as.data.frame(stats_metricS)
# Print the data frame using xtable
print(xtable(stats_sofr))
...
# ----------------------------------------------------------------------
#     axis.text = element_text(color = custom_colors),  # Set axis text color to match plot colors
#     axis.title = element_text(color = custom_colors)  # Set axis title text color to match plot colors


print(securedrates)
ggsave("C:/Users/Zenobia/Documents/Research/MonetaryPolicy/MonetaryPolicy/Figures/Figures2/EFFRrates.pdf")
ggsave("C:/Users/Zenobia/Documents/Research/MonetaryPolicy/MonetaryPolicy/Figures/Figures2/securedrates.png")
...
# --------------------------------Shocks needs work
#C:\Users\Zenobia\Documents\Research\MonetaryPolicy\MonetaryPolicy\Data\Final data files\shocks.csv
...
stats_rrbp<-data.frame(mean_rrbp,median_rrbp,var_rrbp, kurtosis_rrbp,skew_rrbp)
cv<-sqrt(var_rrbp)/mean_rrbp
...
# EFFR        OBFR        TGCR        BGCR        SOFR  rrbpm.EFFR  rrbpm.OBFR  rrbpm.TGCR  rrbpm.BGCR 
# 108.3327485 104.8128655 104.4385965 105.5754386 108.1339181   2.0740351  -1.4458480  -1.8201170  -0.6832749 
# rrbpm.SOFR 
# 1.8752047
#
...
#stats_metricE<-colMeans(metricE)
# stats_metricS<-colMeans(metricS)
# str(stats_metricE) # after adding moments
# List of 10
# $ EFFR         : num 108
# $ TargetUe     : num 94.2
# $ TargetDe     : num 76.7
# $ PercentileE1 : num 98.1
# $ PercentileE25: num 108
# $ PercentileE75: num 109
# $ PercentileE99: num 119
# $ sd           : Named num 0.473
# ..- attr(*, "names")= chr "PercentileE75"
# $ skew         : Named num -164
# ..- attr(*, "names")= chr "PercentileE75"
# $ kurtosis     : Named num 2
# ..- attr(*, "names")= chr "PercentileE75"
#----------------------------------------
...
# new_row <- data.frame(Name = "David", Age = 28, Score = 88)
# df <- rbind(df, new_row)
# 
...
selected_quantilesE
stats_metricE<-colMeans(selected_quantilesE)

stats_metricnorm<-colMeans(selected_quantilesE)
#stats_metricE<-colMeans(metricE)
# stats_metricS<-colMeans(metricS)
# str(stats_metricE) # after adding moments
# List of 10
...
stats_norm<-colMeans(norm) # like stats_MetricE
stats_adjust<-colMeans(adjust)
stats_covid<-colMeans(covid)
stats_zlb<-colMeans(zlb)
stats_inflation<-colMeans(inflation)

# Norm stats
p2575E<-stats_norm[6]-stats_norm[5]
p0199E<-stats_norm[7]-stats_norm[4]
p2575S<-stats_norm[6]-stats_norm[5]
p0199S<-stats_norm[7]-stats_norm[4]
...
#stats_norm$sd<-c(sd.skew,kurtosis)
inverse_value <- qlnorm(0.75) #  1.963031
print(inverse_value)
sde<-p2575E/(2*inverse_value) #  0.4738036 
sds<-p2575S/(2*inverse_value) #   -9.464567 
...
skewe<-3*(p2575E-2*stats_norm[1])/(2*inverse_value) #  -164.1402 
kurte<-p2575E/(inverse_value*sde) # 2
print(skewe)
print(kurte)


skews<-3*(p2575S-2*stats_norm[1])/(2*inverse_value) #    -165.2238 
kurts<-p2575S/(inverse_value*sde) #   0.04471033 
print(skews)
print(kurts)
...
stats_norm$skew <- skewe
stats_norm$skew <- skews

stats_norm$kurtosis <- kurte
stats_norm$kurtosis <- kurts

str(stats_norm)
str(stats_norm)

# Convert the list to a data frame
stats_norm2 <- as.data.frame(stats_norm)
# Print the data frame using xtable
print(xtable(stats_norm2))
...
# stats_adjust
# Norm stats
p2575E<-stats_adjust[6]-stats_adjust[5]
p0199E<-stats_adjust[7]-stats_adjust[4]
p2575S<-stats_adjust[6]-stats_adjust[5]
p0199S<-stats_adjust[7]-stats_adjust[4]
...
#stats_adjust$sd<-c(sd.skew,kurtosis)
inverse_value <- qlnorm(0.75) #  1.963031
print(inverse_value)
sde<-p2575E/(2*inverse_value) #  0.4738036 
sds<-p2575S/(2*inverse_value) #   -9.464567 
...
skewe<-3*(p2575E-2*stats_adjust[1])/(2*inverse_value) #  -164.1402 
kurte<-p2575E/(inverse_value*sde) # 2
print(skewe)
print(kurte)


skews<-3*(p2575S-2*stats_adjust[1])/(2*inverse_value) #    -165.2238 
kurts<-p2575S/(inverse_value*sde) #   0.04471033 
print(skews)
print(kurts)
...
stats_adjust$skew <- skewe
stats_adjust$skew <- skews

stats_adjust$kurtosis <- kurte
stats_adjust$kurtosis <- kurts

str(stats_adjust)
str(stats_adjust)

# Convert the list to a data frame
stats_adjust2 <- as.data.frame(stats_adjust)
# Print the data frame using xtable
print(xtable(stats_adjust2))

# stats_covid---------------------
# covid stats
p2575E<-stats_covid[6]-stats_covid[5]
p0199E<-stats_covid[7]-stats_covid[4]
p2575S<-stats_covid[6]-stats_covid[5]
p0199S<-stats_covid[7]-stats_covid[4]
...
#stats_covid$sd<-c(sd.skew,kurtosis)
inverse_value <- qlnorm(0.75) #  1.963031
print(inverse_value)
sde<-p2575E/(2*inverse_value) #  0.4738036 
sds<-p2575S/(2*inverse_value) #   -9.464567 
...
skewe<-3*(p2575E-2*stats_covid[1])/(2*inverse_value) #  -164.1402 
kurte<-p2575E/(inverse_value*sde) # 2
print(skewe)
print(kurte)


skews<-3*(p2575S-2*stats_covid[1])/(2*inverse_value) #    -165.2238 
kurts<-p2575S/(inverse_value*sde) #   0.04471033 
print(skews)
print(kurts)
...
stats_covid$skew <- skewe
stats_covid$skew <- skews

stats_covid$kurtosis <- kurte
stats_covid$kurtosis <- kurts

str(stats_covid)
str(stats_covid)

# Convert the list to a data frame
stats_covid2 <- as.data.frame(stats_covid)
# Print the data frame using xtable
print(xtable(stats_covid2))

#stats_zlb---------------------
# zlb stats
p2575E<-stats_zlb[6]-stats_zlb[5]
p0199E<-stats_zlb[7]-stats_zlb[4]
p2575S<-stats_zlb[6]-stats_zlb[5]
p0199S<-stats_zlb[7]-stats_zlb[4]
...
#stats_zlb$sd<-c(sd.skew,kurtosis)
inverse_value <- qlnorm(0.75) #  1.963031
print(inverse_value)
sde<-p2575E/(2*inverse_value) #  0.4738036 
sds<-p2575S/(2*inverse_value) #   -9.464567 
...
skewe<-3*(p2575E-2*stats_zlb[1])/(2*inverse_value) #  -164.1402 
kurte<-p2575E/(inverse_value*sde) # 2
print(skewe)
print(kurte)


skews<-3*(p2575S-2*stats_zlb[1])/(2*inverse_value) #    -165.2238 
kurts<-p2575S/(inverse_value*sde) #   0.04471033 
print(skews)
print(kurts)
...
stats_zlb$skew <- skewe
stats_zlb$skew <- skews

stats_zlb$kurtosis <- kurte
stats_zlb$kurtosis <- kurts

str(stats_zlb)
str(stats_zlb)

# Convert the list to a data frame
stats_zlb2 <- as.data.frame(stats_zlb)
# Print the data frame using xtable
print(xtable(stats_zlb2))

# stats_inflation-------------------
# Norm stats
p2575E<-stats_inflation[6]-stats_inflation[5]
p0199E<-stats_inflation[7]-stats_inflation[4]
p2575S<-stats_inflation[6]-stats_inflation[5]
p0199S<-stats_inflation[7]-stats_inflation[4]
...
#stats_inflation$sd<-c(sd.skew,kurtosis)
inverse_value <- qlnorm(0.75) #  1.963031
print(inverse_value)
sde<-p2575E/(2*inverse_value) #  0.4738036 
sds<-p2575S/(2*inverse_value) #   -9.464567 
...
skewe<-3*(p2575E-2*stats_inflation[1])/(2*inverse_value) #  -164.1402 
kurte<-p2575E/(inverse_value*sde) # 2
print(skewe)
print(kurte)


skews<-3*(p2575S-2*stats_inflation[1])/(2*inverse_value) #    -165.2238 
kurts<-p2575S/(inverse_value*sde) #   0.04471033 
print(skews)
print(kurts)

stats_inflation$sd <- sde
stats_inflation$sd <- sds

stats_inflation$skew <- skewe
stats_inflation$skew <- skews

stats_inflation$kurtosis <- kurte
stats_inflation$kurtosis <- kurts

str(stats_inflation)
str(stats_inflation)

# Convert the list to a data frame
stats_inflation2 <- as.data.frame(stats_inflation)
# Print the data frame using xtable
print(xtable(stats_inflation2))

# --------------------- old
stats_sofr <- as.data.frame(stats_norm)
# Print the data frame using xtable
print(xtable(stats_sofr))
...
# Standard deviation σ≈  75th percentile−25th percentile/2 × inverse value 
# 75th percentile−25th percentile
# inverse_value <- qlnorm(0.75) #  1.963031
# Skewness can be estimated using the assumption of a specific distribution. For a log-normal distribution, skewness (
#     S) is related to the standard deviation ( σ):
# S≈ 3×(75th percentile+25th percentile−2×median)/2.44σ
# The constant 2.44 is an approximation assuming a log-normal distribution.
# 
# Estimate Kurtosis:
#   Similarly, for a log-normal distribution, you can estimate excess kurtosis (K) using the standard deviation (σ):
# K≈(75th percentile−25th percentile/2.44σ
# Find the inverse for p = 0.75
# inverse_value <- qnorm(0.75) # 0.7733726
# print(inverse_value)
#plnorm(q, meanlog = 0, sdlog = 1)
# inverse standard normal cumulative distribution function of 0.75 = 2.44 WRONG 0.6744898
# 75th percentile−25th percentile
...
# > .473073/(2*1.96)
# [1] 0.1206819
# > sde<-p2575E/(2*inverse_value)
# > sde
# PercentileE75 
# 0.473072 

p2575E<-metricE[6]-metricE[5]
p0199E<-metricE[7]-metricE[4]
p2575S<-metricS[5]-metricS[5]
p0199S<-metricS[7]-metricS[4]
...
# ---------- STATS sample volumes
mean_vold <- colMeans(vold)
median_vold <- colMedians(vold)
var_vold <- colVars(vold)
sd_vold<-sqrt(var_vold)
kurtosis_vold <- colkurtosis(vold, pvalue = TRUE)
skew_vold<-colskewness(vold, pvalue = TRUE)
cv_vold<-sqrt(var_vold)/mean_vold
stats_vold<-data.frame(mean_vold,median_vold,sd_vold, kurtosis_vold,skew_vold,cv_vold)
moments_vold<-xtable(stats_vold)
...
# stats episodes
# Sample stats         
metricE<-select(spread, EFFR,TargetUe,TargetDe,PercentileE1,PercentileE25,PercentileE75,PercentileE99)
metricS<-select(spread, SOFR,TargetUe,TargetDe,PercentileS1,PercentileS25,PercentileS75,PercentileS99)
metricE<-metricE*100
metricS<-metricS*100
...
stats_metricE<-colMeans(metricE)
stats_metricS<-colMeans(metricS)
effrtable<-xtable( stats_metricE)
sofrtable<-xtable( stats_metricS)
...
PercentileS25<-spread$PercentileS25
PercentileS75<-spread$PercentileS75
metricS<-data.frame(rrbp[,5],TargetDe,TargetUe, PercentileS25, PercentileS75)
str(metricS)
...
targetE <-ggplot(metricE, aes(x = sdate, y=metricE)) +
  geom_ribbon(aes(ymin =metricE[,2], ymax = metricE[,2]), fill = grey70) +
                    geom_line(aes(y = metricE[,1], color = "EFFR"), linetype ="solid", size = 1.5, alpha = 1.25) + 
                                ...
                                scale_color_manual(values = c("EFFR" = "blue)) +
                               theme_minimal()
 grid.arrange(targetE, quantileE, ncol = 2)
 
 # SOFR
 quantileS<-ggplot(metricE, aes(x = sdate)) +
  geom_ribbon(aes(ymin =metricE[,4], ymax = metricE[,5]), fill = "grey70") +
                    geom_line(aes(y = SOFR, color = "SOFR"), linetype ="solid", size = 1, alpha = 1.25) + 
                                labs(x = "Date", y = "basis points (bp)", color = "Lines") + 
                                scale_color_manual(values = c("SOFR" = "blue"))+
                                theme_minimal()
                                print(quantileE)
...
targetS <-ggplot(metricE, aes(x = sdate, y=metricS)) +
  geom_ribbon(aes(ymin =metricE[,2], ymax = metricE[,2]), fill = grey70) +
                    geom_line(aes(y = metricS[,1], color = "SOFR"), linetype ="solid", size = 1.5, alpha = 1.25) + 
                                labs(x = "Date", y = "basis points (bp)", color = "Lines") + 
                                scale_color_manual(values = c("SOFR" = "blue)) +
                                theme_minimal()
grid.arrange(targetS, quantileS, ncol = 2)
 
#  
# # Create an example plot (you should replace this with your actual plots)
# plot1 <- ggplot(data = mtcars, aes(x = mpg, y = hp)) + geom_point()
# plot2 <- ggplot(data = mtcars, aes(x = mpg, y = qsec)) + geom_point()
# 
...
ggsave("C:/Users/Zenobia/Documents/Research/MonetaryPolicy/MonetaryPolicy/Figures/Figures2/ggdkindex.pdf")
ggsave("C:/Users/Zenobia/Documents/Research/MonetaryPolicy/MonetaryPolicy/Figures/Figures2/ggdkindex.png")
