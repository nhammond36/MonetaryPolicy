...
# rmarkdown::render("C:/Users/Owner/Documents/Upwork/Research/MonetaryPolicy/MPResults/LaTeX/ONrates11182023.Rmd")
#, envir = my_env)
# 
# my_env$bondcoupon <- bondcoupon
# my_data<- my_env$bondcoupon
# str(my_data)
...
spread <- spread %>% 
  mutate(IORR = ifelse(is.na(IORR), 0, IORR))
spread <- spread %>% 
  mutate(RRPONTSYAWARD = ifelse(is.na(RRPONTSYAWARD), 0, RRPONTSYAWARD))
...
spread <- spread %>% 
  mutate(PercentileO1 = ifelse(is.na(PercentileO1), 0, PercentileO1))
# Access the modified PercentileE1 column within the spread data frame
# spread$PercentileO1
spread <- spread %>% 
  mutate(PercentileO25 = ifelse(is.na(PercentileO25), 0, PercentileO25))
spread <- spread %>% 
  mutate(PercentileO75 = ifelse(is.na(PercentileO75), 0, PercentileO75))
spread <- spread %>% 
  mutate(PercentileO99 = ifelse(is.na(PercentileO99), 0, PercentileO99))

spread <- spread %>% 
  mutate(PercentileT1 = ifelse(is.na(PercentileT1), 0, PercentileT1))
# Access the modified PercentileE1 column within the spread data frame
# spread$PercentileT1
spread <- spread %>% 
  mutate(PercentileT25 = ifelse(is.na(PercentileT25), 0, PercentileT25))
spread <- spread %>% 
  mutate(PercentileT75 = ifelse(is.na(PercentileT75), 0, PercentileT75))
spread <- spread %>% 
  mutate(PercentileT99 = ifelse(is.na(PercentileT99), 0, PercentileT99))

spread <- spread %>% 
  mutate(PercentileB1 = ifelse(is.na(PercentileB1), 0, PercentileB1))
# Access the modified PercentileE1 column within the spread data frame
# spread$PercentileB1
spread <- spread %>% 
  mutate(PercentileB25 = ifelse(is.na(PercentileB25), 0, PercentileB25))
spread <- spread %>% 
  mutate(PercentileB75 = ifelse(is.na(PercentileB75), 0, PercentileB75))
spread <- spread %>% 
  mutate(PercentileB99 = ifelse(is.na(PercentileB99), 0, PercentileB99))
...
# Daily data frames overnight rates and volumes -------------------------
...
rrbp <-  spread_no_na[, c("EFFR","OBFR","TGCR","BGCR","SOFR")]
...
# selected_quantiles_namesE <- c("EFFR","VolumeEFFR","TargetUe","TargetDe","PercentileE1","PercentileE25","PercentileE75","PercentileE99")
# selected_quantiles_namesO <- c("OBFR","VolumeOBFR","PercentileO1","PercentileO25","PercentileO75","PercentileO99")
# selected_quantiles_namesT <- c("TGCR","VolumeTGCR","PercentileT1","PercentileT25","PercentileT75","PercentileT99")
# selected_quantiles_namesB <- c("BGCR","VolumeBGCR","PercentileB1","PercentileB25","PercentileB75","PercentileB99")
# selected_quantiles_namesS <- c("SOFR","VolumeSOFR","PercentileS1","PercentileS25","PercentileS75","PercentileS99")
# 
# selected_quantilesE <- spread[, c("EFFR","VolumeEFFR","TargetUe","TargetDe","PercentileE1","PercentileE25","PercentileE75","PercentileE99")]
# selected_quantilesO <- spread[, c("OBFR","PercentileO1","PercentileO25","PercentileO75","PercentileO99")]
# selected_quantilesT <- spread[, c("TGCR","PercentileT1","PercentileT25","PercentileT75","PercentileT99")]
# selected_quantilesB <- spread[, c("BGCR","PercentileB1","PercentileB25","PercentileB75","PercentileB99")]
# selected_quantiless <- spread[, c("SOFR","PercentileS1","PercentileS25","PercentileS75","PercentileS99")]
...
# Assuming sdate is already present in spread_no_na
quantilesE <- spread_no_na[, c("sdate", "EFFR", "VolumeEFFR", "TargetUe", "TargetDe", "PercentileE1", "PercentileE25", "PercentileE75", "PercentileE99")]
...
effrrates <- ggplot(quantilesE, aes(x = sdate)) +
  ...
  geom_line(aes(y = quantilesE[,4], color = "PercentileE1"), linewidth = 1) + 
  geom_line(aes(y = quantilesE[,5], color = "PercentileE25"), linewidth = 1) + 
  geom_line(aes(y = quantilesE[,6], color = "PercentileE75"), linewidth = 1) + 
  geom_line(aes(y = quantilesE[,7], color = "PercentileE99"), linewidth = 1) + 
...
sofrrates <- ggplot(quantilesS, aes(x = sdate)) +
  geom_line(aes(y = quantilesS[,1], color = "SOFR"), linewidth =1) + 
  geom_line(aes(y = quantilesS[,2], color = "PercentileS1"), linewidth = 1) + 
  geom_line(aes(y = quantilesS[,3], color = "PercentileS25"), linewidth = 1) + 
  geom_line(aes(y = quantilesS[,4], color = "PercentileS75"), linewidth = 1) + 
  geom_line(aes(y = quantilesS[,5], color = "PercentileS99"), linewidth = 1) + 
  labs(x = "Date", y = "basis points (bp)") + 
  scale_color_manual(values = c("SOFR" = "black", "PercentileS1" = "yellow", "PercentileS25"= "green","PercentileS75" = "blue","PercentileD99" = "red")) + 
  theme_minimal() +
  scale_y_continuous(breaks = y_breaks, limits = y_limits)
print(sofrrates)
...
grid_sample <- grid.arrange(effrrates, sofrrates, securedrates,ncol = 1)
...
# ------------------------ Sample stats
# colMeans are average median values -----------------------------
