...
#  ``` 
 ...
 # Create the ggplot chart and save it as a variable
# ggplot_chart <- ggplot(data, aes(x = Group, y = Rate)) +
#   geom_bar(stat = "identity") +
#   labs(title = "Rates by Group", x = "Group", y = "Rate")
# 
...
# Copy code
# 
# Step 3: Save the RMarkdown file with a `.Rmd` extension.
# 
# Step 4: Knit the RMarkdown file by clicking the "Knit" button in RStudio or using the `knit()` function. This will generate an HTML output that includes the ggplot chart.
# 
# When you knit the RMarkdown document, it will create an output file (e.g., .html

---# Introduction
Level and volatility of overnight policy and wholesale money market rates move closely together but vary dramatically different central bank policy regimes and other external shocks to these markets.  To manage the Federal Funds rates within its target range, this paper investiages the willingness of the US central bank to offset volatility in its policy rate,
...
> print(result[[2]]) # sebv
          [,1]       [,2]       [,3]      [,4]       [,5]
[1,] 0.1268243 0.10705335 0.14167362 0.1400492 0.10528421
[2,] 0.1366107 0.11064621 0.14119800 0.1396571 0.12352944
[3,] 0.1470317 0.11653708 0.16355764 0.1739493 0.14179146
[4,] 0.0515202 0.06648629 0.07764863 0.1045401 0.03906772
[5,] 0.1243188 0.08974691 0.12046807 0.1213039 0.08634243
[6,] 0.1721229 0.38636386 0.38481799 0.3553826 0.21189063
> print(result[[4]]) # R2adj
         EFFR     OBFR      TGCR      BGCR      SOFR
[1,] 0.999998 0.999997 0.9999937 0.9999936 0.9999951
...
nlag<-1
nw<-1
result <-olsgmm(lhv,rhv,nlag,nw) 

 list_res <- list(bv, sebv, R2v, R2vadj, v, Ftest)  #[1]-[6]
...
'''
'''{r}
k=5  # k=5 r(6,11) = a +b_1 r(6,1)
lhv<- rrbp[6:1710,1:5]
ones_v <- rep(1, times = T-5)
nrow(ones_v)
...
ones_v <- rep(1, times = T-10)
nrow(ones_v)
rhv1<- rrbp[1:1699,1:5]
result <-olsgmmv2(lhv,rhv,nlag,nw)  #constant
#print(list_res)
print(result[[1]]) # bv
print(result[[2]]) # sebv
...
Fama and French (1986a) and Lo and MacKinlay (1987) compare the relative variability of returns (1987) over different horizons using variance ratio tests. Using excess or real returns, returns at long horizons exhibit negative serial correlation if the variance ratio < 1
...
<!-- of returns --> This statistic applies negative weight to autocorrelations up to order 2kj3,followed by increasing positive weight up to lag k, followed by decaying positive weights.7
...
---
title: "Distribution of rates in the adjust period"
output: pdf_document
---

# adjust period
moments_adjust<- all.moments(adjust, order.max=6)
# Create a sample data frame with named rows
...
---
title: "Distribution of rates in the covid period"
output: pdf_document
---
...
---
title: "Distribution of rates in the zlb period"
output: pdf_document
---
...
---
title: "Distribution of rates in the inflation period"
output: pdf_document
---
...
---
title: "Distribution of rates in the sample period 2016-2022"
output: pdf_document
---
moments_rrbp <- all.moments(rrbp, order.max=6)
# Create a sample data frame with named rows
momentsrrbp<-as.data.frame(moments_rrbp,row.names=c('constant','mean','variance','skew','kurtosis','r6','r7'))
