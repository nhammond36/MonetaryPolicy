...
for (t in 1:T) {
  if (!is.na(spread_no_na$TargetUe_EFFR[t]) && !is.na(rrbp[t, 2])) {
    if (spread_no_na$TargetUe_EFFR[t] < rrbp[t, 2]) {
      ...
      # Lower target
      g[t] <- rrbp[t, 2] - spread_no_na$TargetDe_EFFR[t]
    } else {
...
edata<-quantilesE_no_na[,3:5]
rrbp[, 2] <- as.numeric(rrbp[, 2])
spec6 <- ugarchspec(variance.model = list(model = "eGARCH",external.regressors = edata, garchOrder = c(1, 1)))
fit6 <- ugarchfit(spec6, data = rrbp[, 2])
# Chatgpt
spec6 <- ugarchspec(
  variance.model = list(
    model = "eGARCH",
    external.regressors = edata,
    garchOrder = c(1, 1)
  )
)

fit6 <- ugarchfit(spec6, data = rrbp[, 2], solver.control = list(tol = 1e-6, maxit = 2000))


# OR set provide initial values for the model parameters using the setfixed function in the ugarchspec call. 
# This can sometimes help the optimization algorithm converge.
spec6 <- ugarchspec(
  variance.model = list(
    model = "eGARCH",
    external.regressors = edata,
  ...
  fixed.pars = list(alpha1 = 0.1, beta1 = 0.8, gamma1 = 0.1)
)
fit6 <- ugarchfit(spec6, data = rrbp[, 2])
...
algorithm is having difficulty converging to a solution. Here are a few additional tips:
  
#   Normalization of Data:
#   Ensure that your financial data is not too large in magnitude, as this might affect the optimization process. Normalizing or scaling your data can sometimes help in numerical stability.
# 
rrbp[, 2] <- scale(rrbp[, 2])
# Data Inspection:
#   Carefully inspect your financial data for any unusual patterns, outliers, or extreme values that might be causing issues. Outliers can have a significant impact on GARCH models.
# 
# Different Optimization Methods:
#   Experiment with different optimization methods by changing the solver argument in the ugarchspec function. Some alternative values include "hybrid" or "solnp." For example:
#   
spec6 <- ugarchspec(
  variance.model = list(
    model = "eGARCH",
...
fit6 <- ugarchfit(spec6, data = rrbp[, 2], solver = "hybrid")
...
# Stack overchange model https://stats.stackexchange.com/questions/568328/egarch-using-rugarch-package-in-r
garchMod <- ugarchspec(
  variance.model = list(
    model = "eGARCH",
    garchOrder = c(1, 1)),
