...
# Introduction
Level and volatility of overnight policy and wholesale money market rates move closely together but vary dramatically over different central bank policy regimes.   This paper investigates the willingness of the US central bank to offset volatility in its policy rate To manage the Federal Funds rates within its target range,

The Federal Reserve responds to volatility in the federal funds market by adjusting the reserve supply to keep the federal funds rate within its target range  by buying or selling assets from banks. These temoporary adjustments to reserves to offset volatility in the Fed Funds rate When the Federal Reserve changes the level of aggregate reserves by trading securities with banks, these Federal Reserve purchases or sales of assets from banks change the size of the Federal Reserve balance sheet.
...
<!-
and other external shocks to these markets
-->
...
change dramatically from year to year (Figure \ref{fig:volumesample1}).
...
However, the distributions of daily rates rates from 2016 to 2022 (basis points) change dramatically under different poliy regimes  (Figure \ref{fig:v). epochs plots
The time series properties of reference rates differ broadly under the policy regimes during the sample perios
...
The upward sloping conves shape of the normalcy period
...
The Fed manages its policy rate the FFR in a corridor system. At times offsetting volatility in the FFR.



The highest median and overnight rates, occur during the adjustment and inflation policy regimes. 207 to 212 basis points, and 226 to 233 basis points respectively. Valalility is hihest during the inflationary regime, 127.03 to 129.04 basis points.  The coefficient of variation is highest during the inlationary regime: 0.62 to 0.63 basis points.  Simailar comng covid, zlb, and adjustment, 0.34 to 0.48 nasis points. The highest negative skewness during the normalcy period, -13.11.
...
## Distributions of reference rates under different policy regimes
...
-->
The distance of the FFR from the upper and lower FOMC target rates may have an effect on volatility. I may also affect the number of days other overnight rates return to changes in the FFR.
...
Notes:
The Federal Reserve balance-sheet normalization which reduced aggregate reserves between 2017
...
<!--
The error message "Missing number, treated as zero" usually occurs when LaTeX encounters a syntax issue or a command that expects a number but doesn't receive one. In the context of the code you've provided, this error is likely due to the \bottomrule command being placed outside of the tabular environment.

The \bottomrule command should be placed within the tabular environment, at the end of the table, to specify the bottom horizontal rule. The error is likely occurring because LaTeX is interpreting \bottomrule as a standalone command without a proper context.
...
\usepackage{makecell} % For formatting cells within a table
-->
