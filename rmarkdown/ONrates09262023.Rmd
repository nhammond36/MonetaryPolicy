---
title: "FOMC preferences for Managng the Volatility of the FFR"
...
---
<!--
rmarkdown::render("C:/Users/Owner/Documents/Research/MonetaryPolicy/LaTeX/ONrates09232023.Rmd", 
                   output_format = "pdf_document", 
                   output_file = "ONrates09232023.pdf",
                   params = list(title = "Reference rates and FOMC policy", 
                                 ...
                                 https://github.com/nhammond36/MonetaryPolicy
-->
<!--This structure separates different aspects of your code and makes it easier to manage and debug. It also ensures that the data is loaded before you attempt to use it in your visualization.
...
- The third+ code chunk (`{r rrbp}`) is where you use the `rrbp` dataframe  and other dataframes to create `ggplot` chart.
```{r, echo=FALSE}
plot(cars)
```
-->
...
sds<-p2575S/(2*inverse_value) #  0.0105756 
...
<!--
empirical methods from Cochrane inflation blog
Computing long run responses Direct regressions ("local projections") $r_{t+j}=b_{r,j} dp_t$
time varying coefficients for episodes?
ES moment matching
Romer and Romer narrative approach, reading Fed minutes to isolate shocks. analysis of historical episodes and changes in regimes with specific models in mind
regress output and inflaation on shocks  to produce response function, a "local projecton" not a VAR. Fed information source Greenbook forecasts of inflation and output
Source: Romer and Romer shocks (2004) updated by Wieland and Yang (2017)
TASK look at my shocks. Regress rates, volumes on shocks
TASK get Romer shocks 

Comments:
Monetary  policy shocks small fraction of movement in FFR. Most FFR movements in the rule , the reaction to the economy term
See Clarida, Gali, Gertler (2000) estimate rules $$ i_t=\rho i_{t-1} + \fi_\pi \pi_t + \fi_x x_t + u_t.
changes in rule not reponses to shocks the interesting thing, i.e. no $u_t$
Phillips curve $$‪\pi_t=E_t\pi_{t+1}+\kappa x_t.  It was not big negative x that brought down inflation but credibility of new rule that brought down expected inflation
TASK: estimate rules for sample and episodes

Fiscal theory good question the effect of interest rate rise  not accompanied by a change in fiscal poicy. What can Fe do by itself
the zlb is a test of theories fiscal theory is consistent with stable quiet infaltion; old Keynes deflation spiral, new Keynes model sunspot volatility
Emergencie of inflation in 2021 and easing depsite interest rates below inflation validate fiscal throfy vs standard theories

-->

    
# Introduction
Level and volatility of overnight policy and wholesale money market rates move closely together but vary dramatically over different central bank policy regimes. This paper investigates the willingness of the US central bank to offset volatility managing the Federal Funds rates (FFR) within its target range. The FFR is interest rate at which depository institutions or ”banks” lend funds to each other on an overnight basis
...
This paper asks how the Federal Reserve Bank, the bank, temporary adjustment of reserves affects volatility of the FFR and other wholesale overnight money market rates. By trading securities with banks and other counterparites, these Federal Reserve purchases or sales of assets from banks change the size of the Federal Reserve balance sheet. A repo transaction,Federal Reserve purchases of securities adds assets to its balance sheet and issues reserves by crediting the Federal Reserve accounts of the selling banks. Fed is lender?
...
In the recently established repo
...
In 2018 the Fed adopted target rates, upper and  lower boundsd for the FFR. In 2019, the FOMC abandoned active management of scarce reserves and other short term
interest rates and adopted a policy of ample reserves,

Under the Ample Reserves policy, the Federal Reserve manages the FFR within its desired range in a “floor” system through Temporary open market operations (TOMO), short-term repurchase and reverse repurchase agreements. These Short-term repurchase and reverse repurchase agreements are designed to temporarily add or drain
reserves available to the banking system and influence day-to-day trading in the Federal Funds market. r 
...
The IOR  is the Fed’s principal tool for interest rate control.The IOR. The IOR is the ceiling, the ON RRP the floor. Banks have little or no incentive to lend their reserves to other banks at rates lower than the IOR rate. No bank would lend at rates on overnight cash lower than the IOR, the rate they can receive from the Fed. Thus  the IOR is, in effect, a reservation rate for banks’ intermediation activities.
...
## FOMC rate changes, monetary regimes, and events occurred during the six year period of our sample 2016-2022 (Table \ref{table:FOMCratechanges}) . 
...
We examine the time series properties of overnight reference rates and their corespondence with the Federal Funds rate, the FOMC policy rate from 2016 thorugh 2022., <!--The federal funds market consists of domestic unsecured borrowings by depository institutions from other depository institutions and certain other entities, primarily government-sponsored enterprises.
-->
Daily overight rates include the unsecured rates EFFR, OBFR, and secured money market rates, TGCR, BGCR, SOFR. 

The average median daily rates rates of our sample are similar across all overnight rates.  The EFFR and SOFR some 108 basis points while the wholesale money market rates, TGCR and BGCR are 104 basis point.  However we can see how widely these rerefence rates change across the sample ( Figure). All daily rates,measured in basis points, ere plotted againt days between 2016 and 2022. Weekends are not included in this sample of daily rates. (figures) (Table \ref{table:FOMCdailyratesstats} ). 
The estimated standard deviation of the EFFR is 0.47, the SOFR -9.46. Both distributions are highly skewed negatively, -164.14 and -165.22. Kurtosis 2 and 0.4.
Figure note: The rates data are medians.  To estimate the standard deviation, skew, and kurtosis from percentiles.  I have assumed rates follow a log normat distribution.


```{r ,  EFFR and percentiles, echo=FALSE, fig.cap="Overnight rates EFFR percentiles 2016-2022"}   
...
```{r ,  SOFR and percentiles, echo=FALSE, fig.cap="Overnight rates SOFR percentiles 2016-2022"}   
...
```{r , wholesale rates, echo=FALSE, fig.cap="Overnight secured rates TGCR, BGCR 2016-2022"}   
...
```{r ,  dailyrates, echo=FALSE, fig.cap="Overnight rates EFFR, OBFR, TGCR, BGCR, SOFR 2016-2022"}           
...
``` 
```{r, results='asis'}
knitr::xtable(stats_rrbp)
```
```{r. onrates_stats<-xtable(stats_rrbp), echo=FALSE}
knitr::xable(mtcars)
# Convert the list to a data frame
stats_effr <- as.data.frame(stats_metricE)
# Print the data frame using xtable
print(xtable(onrates_stats<-xtable(stats_rrbp)))
```
...
```{r. stats effr, echo=FALSE}
# Convert the list to a data frame
stats_effr <- as.data.frame(stats_metricE)
# Print the data frame using xtable
print(xtable(stats_effr))
```
```{r. stats sofr, echo=FALSE}
stats_sofr <- as.data.frame(stats_metricS)
# Print the data frame using xtable
print(xtable(stats_sofr))
Looking at the dispersion of the rates, the difference between the 99th an one percentile, 20.66 shows greater dispersion in the EFFR than the 75th and 25 percentile, 1.86
Looking at the dispersion of these rates, the difference between the 99th an one percentile, 1.76 shows greater dispersion in the SOFR than the 75th and 25 percentile, 0.04
...
stats_metricE
    ...
    108.33275      94.15205      76.73977      98.08070     107.50819     109.36550     118.74152 
> stats_metricS
         SOFR      TargetUe      TargetDe  PercentileS1 PercentileS25 PercentileS75 PercentileS99 
  108.1339181    94.1520468    76.7397661     0.1573099     0.3906433     0.4321637     1.9204678 
> 

<!--
& EFFR & TargetUe & TargetDe & PercentileE1 & PercentileE25 & PercentileE75 & PercentileE99 & sd & skew & kurtosis \\ 
  \hline
PercentileE75 & 108.33 & 94.15 & 76.74 & 98.08 & 107.51 & 109.37 & 118.74 & 0.47 & -164.14 & 2.00 \\ 
 \hline

 & SOFR & TargetUe & TargetDe & PercentileS1 & PercentileS25 & PercentileS75 & PercentileS99 & sd & skew & kurtosis \\ 
  \hline
PercentileS75 & 108.13 & 94.15 & 76.74 & 102.54 & 146.60 & 109.44 & 120.39 & -9.46 & -165.22 & 0.04 \\ 
-->

## Times series behavior of rates within different FOMC policy regimes
The dynamics of reference rates vary dramatically over different policy episodes and events.  The history of FOMC rate changes (Table) suggested these subsamples of the data to examine for Fed appetite for offsetting risk. the characteristics of the EFFR and all overnight money market rates, in both 1) the whole time series 2016-2022 and 2) and during the periods of normalcy (dates), adjusmtment, coping with covid (dates), the zero lower bound (ZLB), and fighting inflation (dates).
I have broken the daily times series into five different periods correspond to  Forbes description () (date) of changes in monetary policy.  Specific FOMC rate changes in the  FFR that occur within these policy episodes (Table):
...
All five rates within the normalcy period are relatively close to the sample average. Rates during the the adjustment period almost double the sample values. Rates during the covid pandemic are almost 20 percen higher. The most dramatic changes, to be expected, are   during the zero lower bound, -5 to 7 basis point and the recent attempts to lower inflation, some 115 percent of rates in the sample as a whole.

Normalcy 116.00&  104.00& 104.00&  109.50&  116.00\\
Adjustment  212.00&  210.00&  207.50&  207.50&  210.00\\
Covid 127.40&  126.50&  124.84&  124.84&  127.04\\
zlb 7.00&    5.00&   7.00&    5.00&   7.00\\
inflation 233.00&  232.00&  226.00&  226.00&  227.00
...
The distributions of daily rates rates, basis points,their medians and standard deviations change dramatically  within differing FOMC policy regimes (Table \ref{table:FOMCdailyratesstats} ). All daily rates,measured in basis points, ere plotted againt days between 2016 and 2022. Weekends are not included in this sample of daily rates 

Daily overight rates include the unsecured rates EFFR, OBFR, and secured money market rates, TGCR, BGCR, SOFR. The federal funds market consists of domestic unsecured borrowings by depository institutions from other depository institutions and certain other entities, primarily government-sponsored enterprises.


It is the volatility of these rates, as measured by the annual standard deviation of each series which motivates this study of the effect of FOMC policy rates on these rates and preferences of the Federal Reserve Bank for managing the volatility of the Federal Funds rates. Volatility is measured by the annual standard deviation of the series (reference other measures, Guyon).

Figure for transactions, billions of dollars,  (Figure \ref{fig:volumesample1}). 
...
The effective federal funds rate (EFFR),(basis points), a volume-weighted median of overnight federal funds transactions.. summary of the Fed's policy rate, the Federal Funds rate.
The overnight bank funding rate (OBFR), calculated using federal funds transactions, certain Eurodollar transactions, and certain domestic deposit transactions, is a measure of wholesale, unsecured, overnight bank funding costs
...
The Broad General Collateral Rate (BGCR),  a measure of rates on overnight Treasury general collateral repurchase agreement (repo) transactions, serves agents looking for specific cusip numbers.
The Secured Overnight Financing Rate (SOFR) includes all trades in the Broad General Collateral Rate plus bilateral Treasury repurchase agreement (repo) transactions cleared through the Delivery-versus-Payment (DVP) service offered by the Fixed Income Clearing Corporation (FICC), which is filtered to remove a portion of transactions considered “specials”. It is a broad measure of the cost of borrowing cash overnight collateralized by Treasury securities. 
...
<!--
Volatility is a measure of the variation or dispersion of a set of values. In finance, it often refers to the degree of variation of a trading price series. When you have data on the median rate and percentiles (1st, 25th, 75th, and 99th), you can estimate volatility using various statistical measures. Here are a few common methods:

Interquartile Range (IQR):
The interquartile range is the difference between the 75th percentile (Q3) and the 25th percentile (Q1). It provides a measure of statistical dispersion and can be used as a measure of volatility. he IQR provides a measure of the spread or volatility of your data. A larger IQR indicates higher volatility, while a smaller IQR suggests lower volatility.

$IQR=Q3−Q1$

Standard Deviation (SD) or Variance**:
If you assume a normal distribution, you can calculate the standard deviation or variance using the percentiles.

$SD = IQR/1.349$ where 1.349 is an approximation for the standard deviation of a normal distribution within the interquartile range.

Average Absolute Deviation (AAD):
The average absolute deviation is the mean of the absolute differences between each data point and the median.

Range:
The range is the difference between the maximum and minimum values in the dataset. While it's a simple measure, it's sensitive to outliers.
$Range=max−min$
You can also calculate the range between the 1st and 99th percentiles as a measure of the overall data range:

Range = 99th Percentile - 1st Percentile

Choose the appropriate measure based on the properties of your data and what you want to convey with your volatility measure. Additionally, consider the assumptions and limitations of each measure based on the distribution and nature of your data. If you want to consider extreme events, the interquartile range and standard deviation may be more suitable.

Skewness and kurtosis are higher moments that provide additional insights into the shape and distribution of the data, beyond what measures like mean, median, and standard deviation can reveal. Here's how you can calculate skewness and kurtosis for your data:

Skewness:
Skewness measures the asymmetry of the probability distribution of a real-valued random variable. A negative skew indicates that the left tail of the distribution is longer or fatter than the right tail, and a positive skew indicates the opposite.

Skewness=3(Mean−Median)/Standard deviation

Kurtosis:
Kurtosis measures the "tailedness" of the probability distribution of a real-valued random variable. It describes the sharpness of the peak of the distribution.

Kurtosis formula (excess kurtosis):

Kurtosis=(Mean-Median)^4/(Standard deviation)^4 - 3 
Skewnes = 
In R, you can use the skewness and kurtosis functions from the moments package to calculate skewness and kurtosis, respectively. Here's a general example of how you might use these functions with your data:

If you have the median and percentiles of the data, you can still estimate skewness and kurtosis using approximations based on the assumed distribution. For example, if you assume a normal distribution (which is often a reasonable assumption for financial data), you can use known properties of the normal distribution to estimate skewness and kurtosis from the percentiles.

Assuming a normal distribution, you can estimate skewness and kurtosis using the following formulas:

Skewness for a Normal Distribution:
Skewness in a normal distribution is 0. So, you can use the fact that skewness is 0 to estimate skewness based on the difference between the 25th percentile (Q1) and the median (Q2) relative to the difference between the median (Q2) and the 75th percentile (Q3).

Skewness formula (approximation):
Skewness≈ (Q3+Q1−2×Median)/(Q3−Q1)

Kurtosis for a Normal Distribution:
Kurtosis in a normal distribution is 3 (excess kurtosis is 0). You can use this property to estimate kurtosis based on the difference between the 1st percentile (Q0.01) and the 99th percentile (Q0.99).

Kurtosis formula (approximation):
Kurtosis≈1+≈ (Q.99+Q.01)/(Q3−Q1)
Remember, these are approximations based on the assumption of a normal distribution. If your data follows a different distribution, these approximations may not be accurate. Always interpret the results with caution and consider the assumptions and limitations.

-->
The average median effective Federal Funds rate (EFFR) for the sample of daily rates from 3/03/2016 through 12/29 2022 is some 108 basis points. The median rate, share by the SOFR, is 91. Mean vs median, discuss meaning of skewnes. Highly negatively slewed,-18.5 bp.  The coefficient of variation,  the standard deviation divided by the mean, is 0.91.  INTERPRET

The coefficient of variation, summarizes the amount of variation as a percentage or proportion of the total. It is useful when comparing the amount of variation for one variable among groups with different means, or among different measurement variables.
Reference: 3.2.1: Coefficient of Variation - Statistics LibreTexts

The overnight money market rates TGCR and BGCR share a similar distribution. Median of the secured rates 

(Table \ref{table:FOMCdailyratesstats} ). Here 
stats_rrbp
     mean_rrbp median_rrbp  sd_rrbp kurtosis      p.value  skewness p.value.1   cv_rrbp
EFFR  108.3327          91 98.15720        1 2.832104e-17 -18.50948         0 0.9060714
OBFR  104.8129          81 99.35037        1 2.832104e-17 -18.50948         0 0.9478834
TGCR  104.4386          79 98.64078        1 2.832104e-17 -18.50948         0 0.9444859
BGCR  105.5754          80 98.66429        1 2.832104e-17 -18.50948         0 0.9345382
SOFR  108.1339          91 98.57034        1 2.832104e-17 -18.50948         0 0.9115580

Volatility of the EFFR as measured by the standard deviation is 98.16 points, similar across unsecured and secured rates, ranging from 98.16 to 99.35 basis points. The standard deviation of all rates, less than their mean, exceed their median value that range from 80 ot 92 basis points. The coefficient of variation ranges from 0.91 to 0.95.
...
The standard deviation of EFFR fell 38 billion in 2017, 87 billion in 2021, and 170 billion in 2022. Volatility in SOFR fell in 2017, 9  billion , and 258 billion in 2021.  Major increases occurred in  2019, 234  billion, and  2022,  168	 billion.
...
\begin{tabular}{rrrrrrrrr}
  \hline
 & mean\_vold & median\_vold & sd\_vold & kurtosis & p.value & skewness & p.value.1 & cv\_vold \\ 
  \hline
VolumeEFFR & 283.80 & 209.50 & 250.61 & 1.00 & 0.00 & -18.51 & 0.00 & 0.88 \\ 
  VolumeOBFR & 339.50 & 312.00 & 225.33 & 1.00 & 0.00 & -18.51 & 0.00 & 0.66 \\ 
  VolumeTGCR & 347.79 & 359.00 & 179.08 & 1.00 & 0.00 & -18.51 & 0.00 & 0.51 \\ 
  VolumeBGCR & 562.05 & 636.00 & 247.06 & 1.00 & 0.00 & -18.51 & 0.00 & 0.44 \\ 
  VolumeSOFR & 383.74 & 103.00 & 405.29 & 1.00 & 0.00 & -18.51 & 0.00 & 1.06 \\ 
   \hline
\end{tabular}
\end{table}

<!__Figures DONT WANT THIS?
1 Median rates
...
Shocks, major events and Fed response to volatility:
...
The Federal Reserve balance-sheet normalization which reduced aggregate reserves between 2017 and September 2019, increased repo rate distortions, the severity of rate spikes, and intraday payment timing stresses, culminating with a significant disruption in Treasury repo markets in mid-September 2019.
-->

After the introduction of IOER in 2008, the target federal funds rate is set at the middle of the Fed’s target range for FFER. Even during the challenging episodes in September16-18, 2019 and March 16-19, 2020, the Fed maintained the EFFR at the upper target:
...
3/19/2020	0.2	103	0.25	0
 Rates vary greatly under these policy episodes (Figure \ref{fig:ggnorm), plots ggnorm, ggadjust, ggcovd, ggzlb, gginlation

* The upward sloping convex shape of the normalcy period
* The relatively flat schedule of daily rates during covid and zero lower bound.
* The linear increasing rates during the Fed's attempt to tame infation


The highest median and overnight rates, occur during the adjustment and inflation policy regimes. 207 to 212 basis points, and 226 to 233 basis points respectively. Valalility is hihest during the inflationary regime, 127.03 to 129.04 basis points.  The coefficient of variation is highest during the inlationary regime: 0.62 to 0.63 basis points.  Similar comng covid, zlb, and adjustment, 0.34 to 0.48 nasis points. The highest negative skewness during the normalcy period, -13.11.

All, except the period of adjust demonstrate standard deviations equal to their median values,  Skewness
means mean not equal to median
...
```{r, zlb, echo=FALSE}ggzlb<-ggplot(zlb, aes(x = zlb_sdate)) +
...
Error: object 'stats_adjust2' not found
> stats_adjust2 <- as.data.frame(stats_adjust)
> # Print the data frame using xtable
> print(xtable(stats_adjust2))
% latex table generated in R 4.3.1 by xtable 1.8-4 package
% Tue Oct  3 11:45:36 2023
\begin{table}[ht]
\centering
\begin{tabular}{rr}
  \hline
...
```{r, adjust stats}
# Convert the list to a data frame
stats_adjust <- as.data.frame(stats_inflation)
# Print the data frame using xtable
print(xtable(stats_adjust))
```

```{r, covid stats}
# Convert the list to a data frame
stats_covid <- as.data.frame(stats_inflation)
# Print the data frame using xtable
print(xtable(stats_covid))
```

```{r, zlb stats}
# Convert the list to a data frame
stats_zlb <- as.data.frame(stats_inflation)
# Print the data frame using xtable
print(xtable(stats_zlb))
```
...
(Table \ref{table:FOMCepochstats} ). epochs plots and stats Here 
stats_norm
     mean_norm median_norm  sd_norm kurtosis      p.value  skewness p.value.1   cv_norm
EFFR  133.9650         116 74.45329        1 2.104439e-09 -13.10732         0 0.5557669
OBFR  127.6355         104 79.14582        1 2.104439e-09 -13.10732         0 0.6200925
TGCR  127.5479         104 78.57923        1 2.104439e-09 -13.10732         0 0.6160762
BGCR  130.4322         110 76.92284        1 2.104439e-09 -13.10732         0 0.5897532
SOFR  134.0993         116 74.98300        1 2.104439e-09 -13.10732         0 0.5591602

stats_adjust
     mean_adjust median_adjust sd_adjust kurtosis    p.value  skewness    p.value.1 cv_adjust
EFFR    199.2031         212.0  16.42122        1 0.09035933 -3.664153 1.870095e-34 0.3737556
OBFR    196.9688         210.0  16.16283        1 0.09035933 -3.664153 1.870095e-34 0.4018192
TGCR    205.2969         207.0  44.51491        1 0.09035933 -3.664153 1.870095e-34 0.3827590
BGCR    205.3438         207.0  44.49870        1 0.09035933 -3.664153 1.870095e-34 0.3746052
SOFR    207.2656         209.5  44.04912        1 0.09035933 -3.664153 1.870095e-34 0.3617725

stats_covid
     mean_covid median_covid sd_covid kurtosis    p.value  skewness    p.value.1  cv_covid
EFFR   125.9817          155 56.14531        1 0.02936657 -4.734451 5.599941e-93 0.4456626
OBFR   125.0917          154 56.68081        1 0.02936657 -4.734451 5.599941e-93 0.4531139
TGCR   123.4404          152 57.53299        1 0.02936657 -4.734451 5.599941e-93 0.4660792
BGCR   123.4679          152 57.47505        1 0.02936657 -4.734451 5.599941e-93 0.4655060
SOFR   125.6330          155 58.34455        1 0.02936657 -4.734451 5.599941e-93 0.4644045

stats_zlb
     mean_zlb median_zlb   sd_zlb kurtosis      p.value  skewness p.value.1    cv_zlb
EFFR 6.464876          7 2.617192        1 6.368494e-06 -9.869312         0 0.4048326
OBFR 5.925620          5 2.842783        1 6.368494e-06 -9.869312         0 0.4797443
TGCR 6.136364          7 2.445683        1 6.368494e-06 -9.869312         0 0.3985558
BGCR 5.064050          5 2.210934        1 6.368494e-06 -9.869312         0 0.4365940
SOFR 6.371901          7 2.176031        1 6.368494e-06 -9.869312         0 0.3415042

stats_inflation
     mean_inflation median_inflation sd_inflation kurtosis    p.value  skewness     p.value.1 cv_inflation
EFFR       207.9442              233     129.0394        1 0.00372241 -6.325206 5.819888e-292    0.6205485
OBFR       207.4365              232     129.8298        1 0.00372241 -6.325206 5.819888e-292    0.6258772
TGCR       202.2589              226     126.8609        1 0.00372241 -6.325206 5.819888e-292    0.6272205
BGCR       202.1980              226     127.0518        1 0.00372241 -6.325206 5.819888e-292    0.6283533
SOFR       203.4365              227     129.0607        1 0.00372241 -6.325206 5.819888e-292    0.6344027

---------- STATS sample rates
```{r rates sample plots}
...
stats_rrbp<-data.frame(mean_rrbp,median_rrbp,var_rrbp, kurtosis_rrbp,skew_rrbp,cv_rrbp)
moments_rrbp<-xtable(stats_rrbp)
```

---------- STATS epochs
```{r rates epochs plots}
...
Limititation.  The articificialty of these regime dates.  Based FOMC recorded rate changes. INSERT TABLE
...
ADD ALL MOMENTS HERE?
<!--
Discuss distributions all moments
Normal distrbution mean, sd
Non mormal  add quintiles ??
-->
...
With weekly data on reserves, rates are higher when reserves are low, and the converse when reserves are high or 'ample' (Tables ~\ref{table:weeklyreserves} and ~\ref{table:statsweeklyratesannual}). Daily reserve data are available only to Federal Reserve staff, NYFed staff recommended this analysis use  the weekly series of reserves Reserve Balances with Federal Reserve Banks: Week Average (WRESBAL) $\$S$ billions normalized by deposits of commercial banks (DPSACBW027SBOG) $\$S$ billions to control for the growth of the banking industry. Both weekly series ending on Wednesday.
...
Note numbers disagree win onratesw_tablev3 Sample 2 7/28/2016-12/29/2022		
...
## Reserves and rates, demand curves?
...
<!-- 
 # -----------------------------------------------
ONweeklyratevolumecovd_reserve not found
...
I apologize for the confusion earlier. It appears that the error is not directly related to the \bottomrule command, but rather to the usage of \multicolumn with the \makecell command.

The error message you're encountering, "Missing number, treated as zero," indicates that LaTeX is expecting a numeric value but is encountering something else (in this case, a command or text). The issue seems to be with \multicolumn{c}{c}{\makecell{Mean}}.

To correct this, you can directly use \makecell without \multicolumn in the header cells. Here's the updated version of your code:
...
```{r, Duffie Frishnamurthy dispersion index echo=FALSE}
T <-nrow(rrbp)
...
Discuss the main findings or trends depicted in the figure. Highlight significant patterns, relationships, or differences. Use specific data points to support your interpretations.
-->
The distance of the FFR from the upper and lower FOMC target rates may have an effect on volatility.
...
-->
The distance of the FFR from the upper and lower FOMC target rates may have an effect on volatility. I may also affect the number of days other overnight rates return to changes in the FFR. ALonso dispersion index from target rates show FFR within its target range throughout the sample.  What about 2019, March 2020?


```{r, Gara dispersion index echo=FALSE}
Equation 1
...
-->
(Figure ~\ref{figure:dispersionEFFRGara})  (Table %~\ref{table:dispersionEFFRGara}) ) 
The graph on the left depicts the EFFR and the upper and lower target rates set by the FOMC. The value of the Gara insex, in red. remains about ?? basis points.  The spikes in the index occur during 2019 and 2020.

(Figure ~\ref{figure:dispersionEFFR})  

The graph on the left depicts the EFFR or SOFR and the upper and lower target rates set by the FOMC.
The graph on the right depicts the EFFR or SOFR and the 35th qnd 75th percentils of the daily EFFR or SOFR .

## Dispersion percentiles and target rates graphs
(Figure ~\ref{figure:dispersionSOFR}) 
INSERT ggplot of median EFFR and upper and lower targets, 25 and 75 pct quintiles

```{r, EFFR dispersion index echo=FALSE}
# quintiles
...
```{r, SOFR dispersion index echo=FALSE}
Where is SOFR code

```
## Dispersion versus mean reversion

We are conjecturing that the absolute differences across rates and or the distance from the FOMC target rates, may affect 1) the speed with secured rates return to the new FFR values and 2) how these measures of dispersion influence the Fed's propensity to offset volatility in the Fed Funds rate.
...
ggplot(metricE, aes(x = sdatett)) +
  ...
  theme_minimal()
'''  
Figure median EFFR, Lower and upper target rates
'''{r}
ggplot(metricE, aes(x = sdatett)) +
  geom_line(aes(y = metricE[,1], color = "EFFR"), linetype ="solid", linewidth = 1.5, alpha = 1.25) + 
  geom_line(aes(y = metricE[,2], color = "TargetDe"),linetype ="dotted" , linewidth = 1) + 
...
## A forth  method of detecting mean reversion involves computing a likelihood ratio test of the null hypothesis of serial independence against a particular alternative
...
$R_1$ denoting the total return in month t. 

This statistic converges to unity if returns are uncorrelated through time. If some of the price variation is due to transitory factors, however, autocorrelations at some lags will be negative and
...
The variance ratio is closely related to earlier tests based on estimated autocorrelations. Using result  (Cochrane 1988) that the ratio of the k-month return variance to k times the one-month return variance is approximately equal to a linear combination of sample autocorrelations, (1) can be written
...
# Fed preference for offsetting volatility
Fed  inclination to accommodate rate volatility
...
-->
\begin{align*}
\epsilon_t = \sigma_𝑡𝑧_t
log(\sig,a^2= \omega + \sum_{i=1}^{p} \alpha_i \epsilon^2_{t−i| + \sum_{j=1}^{q} \beta_j log(\sigma^2_{t-j}
\end{align*} (3)
...
\begin{align*}
\epsilon_t = \sigma_𝑡𝑧_t
log(\sig,a^2= \omega +  \alpha_i \epsilon^2_{t−i| + \sum_{j=1}^{q} \lambda_j log(\sigma^2_{t-j}+
+ \sum_{i=1}^{p} \gamma_i (\frac{| \epsilon_{t−i}|}{ \sigma_{t−i})
\end{align*}
(4)
Journal of Statistical and Econometric Methods, vol. 2, no.3, 2013, 57-73
ISSN: 2051-5057 (print version), 2051-5065(online) 
Scienpress Ltd, 2013 
EGARCH, GJR-GARCH, TGARCH, AVGARCH, 
...
of the threshold EWMA Heston model [4sfjsdlfjsdlkfjkjasdlksjdaskjdk9, Section 2.3] reads as (M4)
