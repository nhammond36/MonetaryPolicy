...
# --------------- READ DATA ------------------
spread<-read.csv("C:/Users/Owner/Documents/Research/MonetaryPolicy/Data/Final data files/NYFedReferenceRates_1142024v3.csv",header=TRUE, sep=",",dec=".",stringsAsFactors=FALSE)
...
# Assuming 'spread_no_na' is your data frame
spread_no_na[, sapply(spread_no_na, is.integer)] <- lapply(spread_no_na[, sapply(spread_no_na, is.integer)], as.numeric)
...
spread_no_na <- spread_no_na %>%
  mutate(across(.cols = -columns_to_exclude, ~ as.numeric(. * 100), .names = "new_{.col}"))
...
spread_no_na <- spread_no_na %>%
  mutate(across(.cols = -columns_to_exclude, ~ as.numeric(. * 100.0)))



spread_no_na[, setdiff(names(spread_no_na), columns_to_exclude)] <- 
  lapply(spread_no_na[, setdiff(names(spread_no_na), columns_to_exclude)], function(x) as.integer(x * 100))
...
spread_no_na <- spread_no_na %>%
  mutate(across(.cols = -columns_to_exclude, ~ round(. * 100)))

spread_no_na <- spread_no_na %>%
  mutate(across(.cols = -columns_to_exclude, ~ as.integer(. * 100)))

spread_no_na[, -columns_to_exclude] <- lapply(spread_no_na[, -columns_to_exclude], function(x) as.integer(x * 100))
str(spread_no_na)
# SOFR > 0 526 180
