...
metricEnorm<-select(spread, EFFR,TargetUe,TargetDe,PercentileE1,PercentileE25,PercentileE75,PercentileE99)
metricE<-metricE*100
```
```{r. SOFR target rates and perentiles dataframe}
metricSnorm<-select(spread, SOFR,TargetUe,TargetDe,PercentileS1,PercentileS25,PercentileS75,PercentileS99)
metricS<-metricS*100
```

```{r, percentile df, echo=FALSE}
...
<-!!  see https://benjaminlouis-stat.fr/en/blog/2020-05-21-astuces-ggplot-rmarkdown/
    how to make a plot p
-->    
    
Histograms and empirical distributions 
```{r, hist and density EFFR, fig.asp=.8 fig.width=7,echo=FALSE
normE<-plot(normal_)
print(normE)
```
```{r, hist and density SOFR, fig.asp=.8 fig.width=7,echo=FALSE
normalS_<-fitdist(rrbp[,5],"norm")
normS<-plot(normalS_)
print(normS)
```

Fitting of the distribution ' normE ' by maximum likelihood 
Parameters:
   estimate Std. Error
 mean 108.29106   2.371974
 sd    98.11497   1.677240
 
 Fitting of the distribution ' normS ' by maximum likelihood 
Parameters:
  estimate Std. Error
mean 108.09234   2.381956
sd    98.52771   1.684296

```{r, hist and density, fig.asp=.8 fig.width=7,echo=FALSE
normS<-plot(normal_)
print(normS)
```
...
```{r ,  EFFR and percentiles Normalcy, echo=FALSE, fig.cap="Overnight rates EFFR percentiles Normalcy"}   
effrrates <- ggplot(metricE, aes(x = sdate)) +
  geom_line(aes(y = metricE[,1], color = "EFFR"), linewidth =1) + 
  geom_line(aes(y = metricE[,4], color = "PercentileE1",alpha = 0.25), linewidth = 1) + 
  geom_line(aes(y = metricE[,5], color = "PercentileE25",alpha = 0.25), linewidth = 1) + 
...
```{r ,  EFFR and percentiles Adjust, echo=FALSE, fig.cap="Overnight rates EFFR percentiles Adjustment period"}   
effrrates <- ggplot(metricE, aes(x = sdate)) +
  geom_line(aes(y = metricE[,1], color = "EFFR"), linewidth =1) + 
  geom_line(aes(y = metricE[,4], color = "PercentileE1",alpha = 0.25), linewidth = 1) + 
  geom_line(aes(y = metricE[,5], color = "PercentileE25",alpha = 0.25), linewidth = 1) + 
...
```{r ,  EFFR and percentiles Covid, echo=FALSE, fig.cap="Overnight rates EFFR percentiles Covid"}   
effrrates <- ggplot(metricE, aes(x = sdate)) +
  geom_line(aes(y = metricE[,1], color = "EFFR"), linewidth =1) + 
  geom_line(aes(y = metricE[,4], color = "PercentileE1",alpha = 0.25), linewidth = 1) + 
  geom_line(aes(y = metricE[,5], color = "PercentileE25",alpha = 0.25), linewidth = 1) + 
...
```{r ,  EFFR and percentiles zlb, echo=FALSE, fig.cap="Overnight rates EFFR percentiles Zero lower bound"}   
effrrates <- ggplot(metricE, aes(x = sdate)) +
  geom_line(aes(y = metricE[,1], color = "EFFR"), linewidth =1) + 
  geom_line(aes(y = metricE[,4], color = "PercentileE1",alpha = 0.25), linewidth = 1) + 
  geom_line(aes(y = metricE[,5], color = "PercentileE25",alpha = 0.25), linewidth = 1) + 
...
```{r ,  EFFR and percentiles inflation, echo=FALSE, fig.cap="Overnight rates EFFR percentiles Inflation targeting"}   
effrrates <- ggplot(metricE, aes(x = sdate)) +
  geom_line(aes(y = metricE[,1], color = "EFFR"), linewidth =1) + 
  geom_line(aes(y = metricE[,4], color = "PercentileE1",alpha = 0.25), linewidth = 1) + 
  geom_line(aes(y = metricE[,5], color = "PercentileE25",alpha = 0.25), linewidth = 1) + 
