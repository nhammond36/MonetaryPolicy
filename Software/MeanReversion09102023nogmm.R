...
# --------------------- GET DATA
# DAILY DATA
filepath<-"C:/Users/Zenobia/Documents/Research/MonetaryPolicy/MonetaryPolicy/Data/"
headers=  read.csv('C:/Users/Zenobia/Documents/Research/MonetaryPolicy/MonetaryPolicy/Data/Final data files/dailyovernightrates090923.csv',header=F, nrows=1) #,as.is=T)
spread <- read.csv('C:/Users/Zenobia/Documents/Research/MonetaryPolicy/MonetaryPolicy/Data/Final data files/dailyovernightrates090923.csv',header=TRUE, sep=",",dec=".",stringsAsFactors=FALSE,skip=4)
colnames(spread)=headers
class(spread)
spread %>% replace(is.na(.),0)
spread$TargetDe %>% replace(is.na(.),0)
spread$TargetUe %>% replace(is.na(.),0)
spread$IOR %>% replace(is.na(.),0)
...
targetdbp<-TargetDe*100
targetubp<-TargetUe*100
...
# whie background no grid
# Define custom color values for the plot
custom_colors <- c("EFFR" = "black", "OBFR" = "blue", "TGCR" = "green", "BGCR" = "orange", "SOFR" = "red")
dailyrates <- ggplot(rrbp, aes(x = sdate)) +
  geom_point(aes(y = rrbp[,1], color = "EFFR"), shape = 16, size = 1) + 
  geom_point(aes(y = rrbp[,2], color = "OBFR"), shape = 16, size = 1) + 
  geom_point(aes(y = rrbp[,3], color = "TGCR"), shape = 16, size = 1) + 
...
plot(c(0, 10), c(0, 10), type = "n", xlab = "", ylab = "") # Create an empty plot to remove the grid
box() # Add a box around the plot (optional)

# Save the plot as a PNG file
png("my_plot.png", width = 800, height = 600, res = 100) # Specify width, height, and resolution
dev.off() # Close the PNG device

# Your plot has been saved as "my_plot.png" with a white background and no grid.
-->  
# --------------------------------Shocks needs work
